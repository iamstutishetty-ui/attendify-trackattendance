  ...
  return (
    <>
      <style>{`
        ...
        .splash-root{position:fixed;inset:0;z-index:9999;display:flex;align-items:center;justify-content:center;overflow:hidden;background:#fff;}
        @media (prefers-color-scheme: dark){.splash-root{background:#000;}}
        ...
        .splash-letter{font-family:'Poppins',sans-serif;font-weight:900;font-size:clamp(2.6rem,9vw,4rem);color:#1E90D8;display:inline-block;opacity:0;transform:scale(0) translateY(20px);animation:splashLetterPop .3s cubic-bezier(.34,1.7,.64,1) forwards;text-shadow:0 0 24px rgba(30,144,216,.4),0 0 50px rgba(30,144,216,.15);}
        ...
        .splash-glow{position:absolute;width:200px;height:200px;border-radius:50%;border:2px solid rgba(30,144,216,.5);opacity:0;animation:splashRingBurst .7s ease-out forwards;animation-delay:2.15s;pointer-events:none;}
        @keyframes splashRingBurst{0%{transform:scale(.3);opacity:.7;}100%{transform:scale(2.5);opacity:0;}}
      `}</style>
